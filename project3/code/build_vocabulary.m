...
for i = 1:size(image_paths, 1)
    ...
    im = imread(path);
    im = im2single(im);
    [locations, features] = vl_dsift(im, 'size', binSize, 'step', step, 'fast');
    accum_features = [accum_features; features'];
end

[centers, asgn] = vl_kmeans(single(accum_features'), vocab_size);
vocab = centers';
